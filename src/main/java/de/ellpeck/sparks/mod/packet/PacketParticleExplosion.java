package de.ellpeck.sparks.mod.packet;
...
public class PacketParticleExplosion implements IMessage{
    ...
    public static class Handler implements IMessageHandler<PacketParticleExplosion, IMessage>{
        ...
        public IMessage onMessage(final PacketParticleExplosion message, MessageContext ctx){
            Sparks.proxy.scheduleTask(new Runnable(){
                @Override
                public void run(){
                    ...
                    for(int i = 0; i < message.amount; i++){
                        ...
                        Sparks.proxy.spawnMagicParticle(mc.world, x, y, z, motionX, motionY, motionZ, message.color, message.scale, 50, 0F, message.collision);
                    }
                }
            });
            return null;
